...
class Engine:
    def __init__(self):
        ...
        # ===== CALIBRATION FORCE METHOD (NumPy vs Barnes-Hut) =====
        Logger.info("Calibrating force computation method...")
        t_calib_start = time.perf_counter()
        self.force_method_n_threshold = calibrate_force_threshold(self.gravity)
        self.force_method_state = {'last': 'numpy'}
        Logger.info(
            f"Calibration done in {(time.perf_counter() - t_calib_start)*1000:.0f}ms "
            f"-> threshold = {self.force_method_n_threshold}"
        )
    ...
    def _occlusion_cull(self, visible):
        ...
        for circle, sx, sy, r in visible:
            cell = (int(sx // cell_size), int(sy // cell_size))
            occluded = False
            for dx in (-1, 0, 1):
                for dy in (-1, 0, 1):
                    for osx, osy, orad in grid.get((cell[0] + dx, cell[1] + dy), ()):
                        if ((sx - osx) ** 2 + (sy - osy) ** 2) ** 0.5 + r <= orad:
                            occluded = True
                            break
                    if occluded:
                        break
                if occluded:
                    break
            if not occluded:
                kept.append((circle, sx, sy, r))
                grid[cell].append((sx, sy, r))
    ...
    def physics_step(self, dt):
        # physics_step: Perform a single physics step using a fixed timestep.
        #
        # This function advances the simulation by one fixed time interval (dt),
        # ensuring deterministic results regardless of the rendering framerate.
        # Operations performed in this step include:
        #   1. Removing any bodies marked for deletion ("suicide").
        #   2. Computing gravitational forces using a Barnes-Hut quadtree
        #      for efficient N-body interactions (O(n log n) on average).
        #   3. Handling body fusions (collisions), using a spatial hash grid
        #      as a broad-phase filter to efficiently detect nearby pairs.
        #   4. Updating the physics state of each remaining body.
        # The method assumes dt is fixed (self.physics_timestep) and that state.circles
        # contains all current celestial bodies. Time acceleration is accounted for
        # in force and collision calculations.
        #
        # Args:
    ...
    def render(self, alpha):
        ...
        if self.debug_mode:
            ...
            Logger.info(f"render: {t_render_total*1000:.2f}ms")
            print(f"render: {t_render_total*1000:.2f}ms")
